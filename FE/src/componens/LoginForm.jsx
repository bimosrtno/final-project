...
const LoginForm = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        setLoading(true); // Set loading true saat mulai proses login
        try {
            ...
            if (error.response && error.response.status === 401) {
                // Jika akun non aktif
                setResponseMessage('Akun Anda tidak aktif. Silakan hubungi admin.');
            } else {
                setResponseMessage('Login gagal. Silakan coba lagi.'); // Pesan error umum
            }
        } finally {
            setLoading(false); // Set loading false setelah proses selesai (baik sukses/ error)
        }
    };
